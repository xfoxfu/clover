...
class Dashboard extends React.Component<RouteComponentProps<{}> & { state: AppState }, {
    ...
    handleChangeTab = (index: number) => (value: string) => {
        this.setState((state) => ({
    ...
    render() {
        ...
        const makeGuideUi = (title: string, guideItems: GuideItem[], index: number) => (
            <Card style={{ marginTop: '8px' }} title={title}>
                ...
                <Steps current={this.state.currentStep[index]} direction="vertical">
                    {guideItems[this.state.currentOs[index]].descriptions.map((description, stepIndex) =>
                        <Step
                            title={description.title}
                            key={stepIndex}
                            description={this.state.currentStep[index] === stepIndex &&
                                <div>
                                        ...
                                        <p>
                                            {description.links.map((link) => (
                                                <span style={{ paddingRight: '4px' }}>
                                                    <Button type="primary" href={link.href} target="_blank">
                                                        {link.name}
                                                    </Button>
                                                </span>
                                            ))}
                                    ...
                                    >
                                        {this.state.currentStep[index] !==
                                            guideItems[this.state.currentOs[index]].descriptions.length - 1 ?
                                            '前进' : '完成'}
                                    </Button>
